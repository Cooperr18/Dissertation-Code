# CONTENT BIAS TRANSMISSION "SNAPSHOT" VERSION ------------------------
...
content_bias_snapshot <- function(N, mu, s_true, burnin, timesteps, p_value_lvl, n_runs) {
  ...
  sel_variant <- 90  # we choose the focal variant
  
  # Table to store results for the focal variant 
  results <- tibble(
    run = seq_len(n_runs),
    variant = rep(sel_variant, n_runs), # number of variant
  ...
  # Export output
  return(list(
    sel_variant = sel_variant, # variant
    ...
    results = results # focal variant results
  )
  )
}

# Check results
cb_snap_sim <- content_bias_snapshot(N = 100, mu = 0.02, s_true = 0.5,
                                     burnin = 500, timesteps = 500, 
                                     p_value_lvl = 0.05, n_runs = 100)
results <- cb_snap_sim$results
results

fit_results <- cb_snap_sim$fit_results
fit_results$sig == "selection"

results_table_cb_snapshot <- tibble(
  "Focal Variant" = cb_snap_sim$sel_variant,
  N  = cb_snap_sim$N,
  mu = cb_snap_sim$mu,
  ...
  "Mean p-value" = mean(results$fit_p, na.rm =TRUE),
  SSR = cb_snap_sim$SSR,
  FNR = cb_snap_sim$FNR,
...
params_cb_snapshot <- list(
  list(N=100, mu=0.01, s_true = 0.1, burnin=500, timesteps=500, p_value_lvl=0.05, n_runs=100),
  list(N=100, mu=0.01, s_true = 0.25, burnin=500, timesteps=500, p_value_lvl=0.05, n_runs=100),
  list(N=100, mu=0.01, s_true = 0.5, burnin=500, timesteps=500, p_value_lvl=0.05, n_runs=100),
  list(N=100, mu=0.01, s_true = 0.75, burnin=500, timesteps=500, p_value_lvl=0.05, n_runs=100)
)

all_results_cb_snapshot <- map_dfr(params_cb_snapshot, ~ {
  sim <- do.call(content_bias_snapshot, args = .x)
  tibble("Focal Variant" = sim$sel_variant,
         N  = .x$N,
         mu = .x$mu,
         "S" = .x$s_true,
         burnin = .x$burnin,
